...
fn bench_parse_simple_select(c: &mut Criterion) {
    c.bench_function("parse_simple_select", |b| {
        b.iter(|| {
            Parser::parse("SELECT * FROM users WHERE id = 1").unwrap()
        })
    });
}
...
fn bench_parse_window(c: &mut Criterion) {
    let query = "SELECT RANK() OVER (PARTITION BY department ORDER BY salary DESC) as rank FROM employees";
...
fn bench_parse_delete(c: &mut Criterion) {
    ...
    c.bench_function("parse_delete", |b| {
        b.iter(|| {
            Parser::parse(query).unwrap()
        })
    });
