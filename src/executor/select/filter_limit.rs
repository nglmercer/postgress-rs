...
impl<'a> ExecContext<'a> {
    pub fn execute_select_without_from(&self, select_list: &[SelectItem]) -> Vec<(ItemPointerData, Row)> {
    ...
    pub fn apply_offset(&self, rows: Vec<(ItemPointerData, Row)>, offset_expr: &Expr) -> Vec<(ItemPointerData, Row)> {
    ...
    pub fn apply_where(&self, rows: Vec<(ItemPointerData, Row)>, where_clause: &Expr) -> Vec<(ItemPointerData, Row)> {
        rows.into_iter().filter(|(_tid, row)| {
            crate::server::evaluate_where(where_clause, row, self.tuple_desc.as_ref())
        }).collect()
    }

    pub fn apply_having(&self, rows: Vec<(ItemPointerData, Row)>, having: &Expr) -> Vec<(ItemPointerData, Row)> {
        rows.into_iter().filter(|(_tid, row)| {
    ...
    pub fn apply_distinct_on(&self, rows: Vec<(ItemPointerData, Row)>, on_exprs: &[Expr]) -> Vec<(ItemPointerData, Row)> {
        let mut seen = std::collections::HashSet::new();
    ...
    pub fn apply_order_by(&self, mut rows: Vec<(ItemPointerData, Row)>, order_by: &[OrderByItem]) -> Vec<(ItemPointerData, Row)> {
        rows.sort_by(|a, b| {
