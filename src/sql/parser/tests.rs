...
fn test_create_table() {
    let stmt =
        Parser::parse("CREATE TABLE users (id INT PRIMARY KEY, name TEXT NOT NULL)").unwrap();
    match stmt {
...
fn test_select_with_join() {
    let stmt = Parser::parse("SELECT u.id, o.total FROM users u JOIN orders o ON u.id = o.user_id")
        .unwrap();
    match stmt {
        Statement::Select(sel) => {
...
fn test_select_with_order_limit() {
    let stmt = Parser::parse("SELECT * FROM users ORDER BY name ASC NULLS LAST LIMIT 10 OFFSET 20")
        .unwrap();
    match stmt {
        Statement::Select(sel) => {
...
fn test_select_with_case() {
    let stmt = Parser::parse("SELECT CASE WHEN age > 18 THEN 'adult' ELSE 'minor' END FROM users")
        .unwrap();
    match stmt {
        Statement::Select(sel) => {
            assert_eq!(sel.select_list.len(), 1);
...
fn test_insert_on_conflict_do_nothing() {
    let stmt =
        Parser::parse("INSERT INTO users (id, name) VALUES (1, 'Alice') ON CONFLICT DO NOTHING")
            .unwrap();
    match stmt {
        Statement::Insert(ins) => {
...
fn test_extract_month() {
    ...
    match stmt {
        Statement::Select(sel) => match &sel.select_list[0] {
...
fn test_date_trunc() {
    ...
    match stmt {
        Statement::Select(sel) => {
            assert_eq!(sel.select_list.len(), 1);
            match &sel.select_list[0] {
                SelectItem::Expr(Expr::DateTrunc {
                    field,
                    source,
                    zone,
                }) => {
...
fn test_date_trunc_with_zone() {
    let stmt =
        Parser::parse("SELECT DATE_TRUNC('day', ts AT TIME ZONE 'UTC') FROM events").unwrap();
    match stmt {
        Statement::Select(sel) => match &sel.select_list[0] {
            SelectItem::Expr(Expr::DateTrunc {
                field,
    ...
    match stmt {
        Statement::Select(sel) => match &sel.select_list[0] {
            SelectItem::Expr(Expr::AtTimeZone { expr, zone }) => {
                ...
                assert!(
                    matches!(zone.as_ref(), Expr::Literal(Literal::String(s)) if s == "US/Eastern")
                );
            }
            other => panic!("expected AtTimeZone, got {:?}", other),
        },
...
fn test_multiple_date_functions() {
    let stmt =
        Parser::parse("SELECT NOW(), CURRENT_DATE, CURRENT_TIMESTAMP, LOCALTIME, LOCALTIMESTAMP")
            .unwrap();
    match stmt {
        Statement::Select(sel) => {
...
fn test_update_with_date_cast() {
    let stmt =
        Parser::parse("UPDATE events SET event_date = '2023-12-25'::date WHERE id = 1").unwrap();
    match stmt {
...
fn test_window_with_preceding_bound() {
    let stmt =
        Parser::parse("SELECT id, SUM(amount) OVER (ORDER BY id ROWS 5 PRECEDING) FROM orders")
            .unwrap();
    match stmt {
        Statement::Select(sel) => {
            assert_eq!(sel.select_list.len(), 2);
...
fn test_create_sequence_with_no_cycle() {
    let stmt = Parser::parse("CREATE SEQUENCE test_seq NO CYCLE NO MINVALUE NO MAXVALUE").unwrap();
    match stmt {
        Statement::CreateSequence(seq) => {
...
fn test_create_type_composite() {
    let stmt =
        Parser::parse("CREATE TYPE address AS (street TEXT, city VARCHAR(100), zip_code CHAR(5))")
            .unwrap();
    match stmt {
...
fn test_create_type_enum() {
    let stmt =
        Parser::parse("CREATE TYPE status AS ENUM ('active', 'inactive', 'pending')").unwrap();
    match stmt {
        Statement::CreateType(ct) => {
...
fn test_create_type_range() {
    let stmt =
        Parser::parse("CREATE TYPE float_range AS RANGE (SUBTYPE = DOUBLE PRECISION)").unwrap();
    match stmt {
        Statement::CreateType(ct) => {
...
fn test_merge_simple() {
    ...
    match stmt {
        Statement::Merge(mg) => {
            ...
            assert_eq!(
                mg.source,
                MergeSource::Table(ObjectName::new(vec!["source".to_string()]))
            );
...
fn test_array_cast() {
    ...
    match stmt {
        Statement::Select(sel) => {
            assert_eq!(sel.select_list.len(), 1);
            match &sel.select_list[0] {
                SelectItem::Expr(Expr::TypeCast { data_type, .. }) => match data_type {
...
fn test_select_with_between() {
    let stmt = Parser::parse("SELECT * FROM users WHERE age BETWEEN 18 AND 65").unwrap();
    match stmt {
        Statement::Select(sel) => {
            assert!(sel.where_clause.is_some());
            let where_clause = sel.where_clause.as_ref().unwrap();
...
fn test_create_materialized_view() {
    let stmt =
        Parser::parse("CREATE MATERIALIZED VIEW my_view AS SELECT id, amount FROM orders").unwrap();
    match stmt {
